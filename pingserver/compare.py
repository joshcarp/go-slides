...
def generate_graph(filename, request_range):
    ...
    plt.plot(data.numRequests.tolist(), data.java_90.div(1000).tolist(), "^", label = "Java 90th percentile", color='#ff9721')
    plt.plot(data.numRequests.tolist(), data.go_50.div(1000).tolist(), "o", label = "Go Median", color = '#4f92ff')
    plt.plot(data.numRequests.tolist(), data.go_90.div(1000).tolist(), "^", label = "Go 90th percentile", color = '#4f92ff')
    bottom, top = plt.ylim()
    # plt.ylim(top = top + 20)
